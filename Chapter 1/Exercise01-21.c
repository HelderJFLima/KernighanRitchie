...
main()                              /* entab: replace strings of blanks by the minimum number of tabs and */
{                                   /*        blanks to achieve the same spacing. */
    ...
    while( (c = getchar()) != EOF )
    {
        ...
        else if(c != ' ' && state == BLANK)
        {
            ...
            tabstring(tabstr, TAB, pos, nblk, MAXSIZE); /* Make a string of tabs and blanks. */
...
void tabstring(char str[], int tab, int pos, int blknum, int lim)   /* Make an equivalent string of tabs and blanks from */
{                                                                   /* one made of just blanks. */
    int i, nblk, ntab, rpos;

    rpos = pos - (pos / tab) * tab;             /* Relative position of the last character (0, 1, 2, ..., tab - 1) */

    if(rpos != 0 && blknum < tab - rpos)        /* Out of tab stop; not enough blanks to reach the next one */
    {
        ntab = 0;                                   /* Number of tabs to make the new string */

        nblk = blknum;                              /* Number of blanks */
    }
    else if(rpos != 0 && blknum >= tab - rpos)  /* Out of tab stop; but the blanks are enough */
    {
        ntab = ( blknum - (tab - rpos) ) / tab + 1;

        nblk = blknum - (tab - rpos) - (ntab - 1) * tab;
    }
    else if(blknum < tab)                       /* On the tab stop; not enough blanks to the next */
    {
        ntab = 0;

        nblk = blknum;
    }
    else if(blknum >= tab)                      /* On the tab stop; blanks are enough */
    {
        ntab = blknum / tab;
    ...
    for(i = 0; i < ntab && i < lim - 1; ++i)    /* Make the string of tabs and blanks. */
        str[i] = '\t';
